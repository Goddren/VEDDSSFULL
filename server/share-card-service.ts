...
export async function generateShareCard(data: ShareCardData): Promise<Buffer> {
  ...
  if (data.unifiedSignal) {
    ...
    roundRect(ctx, PADDING, yPos, CARD_WIDTH - PADDING * 2, 160, 16);
    ctx.fill();

    const signal = data.unifiedSignal;
    const directionColor = signal.direction.toUpperCase() === 'BUY' ? '#22c55e' : 
                          signal.direction.toUpperCase() === 'SELL' ? '#ef4444' : '#f59e0b';
