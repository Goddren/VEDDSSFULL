...
function getAssetSpecificConfig(symbol: string): AssetSpecificConfig {
  ...
  // Equity Index Configuration (US30, NAS100, SPX500, UK100, GER40, DAX, etc.)
...
// Vision-capable by default. userModelPreferences is an in-memory Map, so every
// deploy resets every user to this value — which meant the default WAS the
// effective model most of the time, not a rarely-hit fallback.
//
// It used to be 'openai/gpt-oss-20b', which is textOnly: it cannot read a chart
// image, and measured on 2026-09-22 it returned EXACTLY 75 on all 298 calls —
// a constant, not an assessment. Against a minConfidence of 85 that made the
// gate unpassable, so the engine confirmed nothing no matter how good the setup.
// gpt-4o-mini is what VISION_FALLBACK.openrouter already promotes text-only
// models to, and the comment there describes it as scoring trade confirmations
// reliably; using it directly means confirmation starts on a capable model
// instead of depending on the promotion path catching every caller.
