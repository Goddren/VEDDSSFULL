...
var init_storage = __esm({
  "server/storage.ts"() {
    ...
    `);
      ...
      async resolveConfirmationOutcome(userId, symbol, direction, tradeOutcome, actualPips, closeTime) {
        const parsed = closeTime ? new Date(closeTime) : null;
        const anchor = parsed && !isNaN(parsed.getTime()) ? parsed : /* @__PURE__ */ new Date();
        const GRACE_MS = 5 * 60 * 1e3;
        const windowStart = new Date(anchor.getTime() - 24 * 60 * 60 * 1e3);
        ...
        if (rows.length > 0) {
          await db.update(aiConfirmationOutcomes).set({ tradeOutcome, actualPips, closedAt: anchor }).where(eq(aiConfirmationOutcomes.id, rows[0].id));
          return true;
        }
        return false;
      }
...
__export(cryptocom_scanner_exports, {
  getDefiUniverseEntry: () => getDefiUniverseEntry,
...
async function closePosition(userId, trade, currentPrice, reason) {
  ...
  try {
    ...
    if (venue === "defi") {
      ...
      const connection = await storage.getUserCryptocomConnections(userId).then((c) => c.find((x) => x.id === trade.connectionId)).catch(() => void 0);
      if (!connection) {
...
async function manualCloseCryptoTrade(userId, tradeId) {
  try {
    ...
    const cfgForClose = await storage.getUserCryptocomEngineConfig(userId).catch(() => void 0);
    const closeChain = cfgForClose?.defiChain || "base";
    let px = 0;
    try {
...
async function reconcileOpenQuantities() {
  ...
  for (const uid of userIds) {
    const trades = await storage.getOpenCryptocomEngineTrades(uid).catch(() => []);
    ...
    const { decryptApiSecret: decryptApiSecret2 } = await Promise.resolve().then(() => (init_cryptocom(), cryptocom_exports));
    const { pool: pool2 } = await Promise.resolve().then(() => (init_db(), db_exports));
    const { rows } = await pool2.query(`SELECT encrypted_private_key k FROM defi_hot_wallets WHERE user_id=$1 AND is_active=true LIMIT 1`, [uid]);
    if (!rows[0]) continue;
    let wallet;
    try {
      wallet = addressFromPrivateKey2(decryptApiSecret2(rows[0].k));
    } catch {
      continue;
    }
...
var MIN_SCAN_INTERVAL_MS, lastScanAt, MAX_SYMBOLS_PER_CYCLE, scanCursor, defiUniverse, STRATEGY_RUNNERS, AUTO_STRATEGIES, sessionPeakEquity, started, scanInFlight, _skippedTicks, _scansCompleted, WORKER_ID, _hbBooted, CRYPTO_RUN_LOCK_KEY, _holdsRunLock, STALE_AFTER_MS;
