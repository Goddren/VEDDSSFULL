...
- **Social Share Card System**: Generates branded share cards with AI Trading Vault branding, EA details, multi-timeframe analysis, and daily devotional scriptures for social media promotion. VEDD SS AI progress sharing generates a 1080x1080 branded card with progress bar, growth stats, and VEDD logo, plus AI-generated social media posts via GPT-4o with dollar-amount sanitization.
...
- **VEDD SS AI Self-Learning Brain Engine**: Autonomous self-learning trading engine that analyzes ALL historical trades (DB + MT5 closed trades) to build deep per-pair knowledge including win rates, best hours/days/sessions, direction biases, high-confidence trade accuracy, streak patterns, and risk-reward ratios. Brain stores learned insights and feeds them into strategy generation and autonomous signal creation. API: `POST /api/vedd-brain/learn`, `GET /api/vedd-brain/status`, `POST /api/vedd-brain/autonomous-signals`, `GET /api/vedd-brain/strategy-modes`.
- **Autonomous Signal Generator**: AI generates proactive trade signals using learned patterns and collected market data WITHOUT requiring live MT5 data feed. Supports 5 HFT strategy modes: Scalping (3-8 pip targets, 10-20+ trades/day), Momentum Surfing (15-40 pips, ride breakouts), Session Breakout (London/NY opens), Aggressive Compound (all strategies combined), and Sniper Mode (2-4 surgical trades). Signals include entry zones, SL/TP, lot sizes, hold times, confidence scores, and learned-edge explanations.
- **HFT Strategy Modes**: Weekly strategy generation now accepts a `strategyMode` parameter (scalping/momentum/session_breakout/aggressive/sniper) that shapes the AI prompt for different high-frequency trading approaches. The AI prompt includes brain-learned insights and pair-specific knowledge for maximum edge. Frontend strategy mode selector with risk level badges.
