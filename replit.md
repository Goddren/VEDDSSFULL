...
- **ICT "One Setup For Life" + CRT + SMC Strategy Layer**: Server-side `server/utils/ictMacroUtils.ts` (ICT macro windows, premium/discount zones, stop hunt, CRT detection) and `server/utils/smcUtils.ts` (BOS/CHOCH, Fair Value Gaps, Order Blocks/Breakers, Equal Highs/Lows liquidity pools, Wyckoff phases). Computed on every 2nd AI confirmation call. AI returns `smcVerdict` (CONFIRM/REQUIRE_BETTER_PRICE/PASS), `smcQuality` (HIGH/MEDIUM/LOW), `smcReason`, plus ICT fields. SMC badges with star ratings and ICT badges surface in the Action Feed.
- **Lemon Squeezy USD Subscriptions**: Full Lemon Squeezy integration alongside Stripe. Service in `server/lemonsqueezy.ts`. Each paid plan shows a yellow "Pay USD" button that creates a LS checkout session. Webhook at `POST /api/lemonsqueezy/webhook` handles `subscription_created/updated/cancelled` events and updates the user's `lsSubscriptionId`, `lsCustomerId`, `subscriptionPlanId`, and `subscriptionStatus`. Admin setup panel on the subscription page (collapsible, logged-in users only) allows entering and saving Lemon Squeezy variant IDs per plan. `ls_variant_id` column added to `subscription_plans` table; `ls_subscription_id` and `ls_customer_id` added to `users` table. Store: VEDDBRAND (store ID 310446) at vedd.lemonsqueezy.com.

### Technical Implementations
- **Backend**: Node.js with Express, TypeScript.
- **Database**: PostgreSQL with Drizzle ORM.
- **Authentication**: Passport.js with local strategy and Phantom wallet login for token-gated membership.
- **AI Integration**: Universal AI Client system supporting OpenAI, Groq (Llama-3.3-70b), Anthropic (Claude 3.5 Sonnet), Google AI (Gemini 1.5 Pro), and Mistral (Mistral Large). All autonomous engine calls — live trading, Sol Engine, brain signals, news sentiment, chart analysis, weekly strategy, social posts, and all user-facing AI routes — automatically use the first active provider key the user has saved, falling back to the platform OpenAI key. Validated through `/api/user-api-keys/validate` endpoint. Header quick-switch dialog (all 5 providers) and full AI API Keys management page at `/ai-api-keys`.
...
- **News-Aware AI Confirmation**: AI Second Opinion system fetches real-time news sentiment and economic events, integrating a 14-point institutional checklist for confirmation decisions. Live Trading Engine also integrates Finnhub news headlines, economic calendar events, and market sentiment into every scan cycle for news-aware trade decisions.
- **Volume-Aware Trading**: Live Engine computes relative volume metrics per pair (surging/above_average/average/below_average/dry), prioritizes high-volume pairs for entries, avoids dry-volume pairs, and identifies optimal trading windows (London/NY overlap).
