...
const PITCH_DECKS: Deck[] = [
  ...
  },
  {
    ...
    slides: [
      ...
      { title: "FX SS AI Engine", bullets: ["Trades TradeLocker, MT5 & DXtrade", "AI confirms every setup before it fires", "Auto risk-sizing + trailing protection"],
...
const SETUP_DECKS: Deck[] = [
  {
    ...
    slides: [
      ...
      { title: "Step 3 — Connect your broker", bullets: ["Primary: TradeLocker", "Then MT5, then DXtrade (Velotrade)", "You only need one — read-only first, then enable trading"],
        notes: "TradeLocker is the main, recommended broker — start there. MT5 is the next option (needs the EA), and DXtrade/Velotrade is the third. Connect read-only first so they see it working before enabling live trades.",
        link: { label: "Connect TradeLocker", href: "/mt5-chart-data" } },
  ...
  },
  {
    ...
    summary: "TradeLocker first, then MT5, then DXtrade (Velotrade) — connect and enable trading.",
    slides: [
      { title: "Which broker do you have?", bullets: ["1. TradeLocker — recommended", "2. MetaTrader 5 (MT5)", "3. DXtrade — broker: Velotrade"],
        notes: "TradeLocker is our main, recommended broker — start there. MT5 is second (needs the EA bridge), DXtrade/Velotrade is third. You only need one to start.",
        link: { label: "Open the FX engine", href: "/mt5-chart-data" } },
      { title: "TradeLocker — connect (start here)", bullets: ["Enter your TradeLocker login", "Pick your environment + server", "Connects read-only first"],
        notes: "This is the primary path. Use their TradeLocker credentials, select the right environment (demo/live) and server. It connects read-only so they can confirm the balance before enabling trades.",
        link: { label: "Connect TradeLocker", href: "/mt5-chart-data" } },
      { title: "MT5 — install the EA", bullets: ["Download the VEDD EA", "Drop it on a chart in MT5", "It links your account to VEDD"],
        notes: "Second option. For MT5, the Expert Advisor is the bridge. Walk them to the EA download and how to attach it. Once it reports in, the engine can trade.",
      ...
      { title: "DXtrade (Velotrade) — connect", bullets: ["Enter username + password", "Domain is 'default'", "Connects read-only first"],
        notes: "Third option. For DXtrade, use the Velotrade login. Domain stays 'default'. It connects read-only so they can confirm the balance is right before enabling trades.",
