            ...
            <div className="grid grid-cols-2 gap-3">
              {SETUP_TOPICS.map(t => (
                <button key={t.id} onClick={() => loadGuide(t.id)}
                  className={`rounded-xl p-4 border text-left transition-all hover:scale-[1.02] ${t.bg} ${setupTopic === t.id ? "ring-2 ring-purple-500/50" : ""}`}>
                  <div className={`mb-2 ${t.color}`}>{t.icon}</div>
                  <p className={`text-sm font-bold ${t.color}`}>{t.label}</p>
                  <p className="text-[10px] text-gray-500 mt-0.5">Step-by-step guide</p>
                </button>
              ))}
            </div>
                ...
                <div className="p-4 space-y-3">
                  ...
                  {guide.steps.map((step, i) => (
                    <div key={i} className="flex gap-3">
                      ...
                      <p className="text-[12px] text-gray-200 leading-relaxed pt-0.5">{step}</p>
                    </div>
                  ))}

                  {guide.tips.length > 0 && (
                    <>
                      <p className="text-[10px] text-gray-500 uppercase tracking-wider mt-4">Pro Tips</p>
                  ...
                  {guide.nextPage && (
                    <a href={guide.nextPage}
                      className="mt-3 w-full flex items-center justify-center gap-2 bg-purple-600/30 hover:bg-purple-600/50 border border-purple-500/40 text-purple-100 text-xs font-bold rounded-lg py-2.5">
                      <ExternalLink className="w-3.5 h-3.5" />
                      Go to {guide.nextPage}
                    </a>
                  )}

                  <button onClick={() => { send(`Walk me through ${SETUP_TOPICS.find(t => t.id === setupTopic)?.label} step by step. I'm a beginner.`); setActiveTab("bot"); }}
